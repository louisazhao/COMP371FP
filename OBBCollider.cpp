...
OBBCollider::OBBCollider(const glm::vec3& origin, float length, float width,float userScale):
center(origin),scale(userScale),lengthOfTheBody(length),widthOfTheBody(width){};


float OBBCollider::calculateLength() const
{
    float result;
...
bool OBBCollider::inRange(const OBBCollider& other) const
{
...
glm::vec3 OBBCollider::getCenter() const
{
    return center;
}
